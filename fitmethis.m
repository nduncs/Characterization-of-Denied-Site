function [F,distribution,x,y]= fitmethis(data,varargin)
...
if strcmp('on',fig)
	if strcmp('binomial',F(best).name)
		[x,y] = plotfitdist(data,F(best).name,F(best).par,dtype,'ntrials',ntrials(best));
	else
		[x,y] = plotfitdist(data,F(best).name,F(best).par,dtype);
	end
end
